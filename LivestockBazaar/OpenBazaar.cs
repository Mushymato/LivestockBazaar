...
internal static class OpenBazaar
{
    ...
    internal static Func<GameLocation, string, ShopOwnerData?, bool> ShowShopDelegate =>
        (!BazaarMenu.StardewUIEnabled || ModEntry.Config.VanillaLivestockMenu)
            ? ShowVanillaAnimalShop
            : BazaarMenu.ShowFor;
        ...
        try
        {
            ...
            harmony.Patch(
                original: AccessTools.DeclaredMethod(
                    typeof(GameLocation),
                    nameof(GameLocation.ShowAnimalShopMenu)
                ),
                prefix: new HarmonyMethod(
                    typeof(OpenBazaar),
                    nameof(GameLocation_ShowAnimalShopMenu_Prefix)
                )
            );
            ...
            harmony.Patch(
                original: AccessTools.DeclaredMethod(
                    ...
                    nameof(PurchaseAnimalsMenu.setUpForReturnAfterPurchasingAnimal)
                ),
                transpiler: new HarmonyMethod(
                    typeof(OpenBazaar),
                    nameof(PurchaseAnimalsMenu_ReturnToPreviousLocation_Transpiler)
                )
            );
            harmony.Patch(
                original: AccessTools.DeclaredMethod(
                    typeof(PurchaseAnimalsMenu),
                    nameof(PurchaseAnimalsMenu.setUpForReturnToShopMenu)
                ),
                transpiler: new HarmonyMethod(
                    typeof(OpenBazaar),
                    nameof(PurchaseAnimalsMenu_ReturnToPreviousLocation_Transpiler)
            ...
            harmony.Patch(
                original: AccessTools.DeclaredMethod(
                    typeof(PurchaseAnimalsMenu),
                    nameof(PurchaseAnimalsMenu.marnieAnimalPurchaseMessage)
                ),
                transpiler: new HarmonyMethod(
                    typeof(OpenBazaar),
                    nameof(PurchaseAnimalsMenu_marnieAnimalPurchaseMessage_Transpiler)
                )
            );
        }
        catch (Exception err)
    ...
    private static void Console_ShowLivestockShop(string command, string[] args)
    {
        ...
        ShowShopDelegate(Game1.currentLocation, shopName, null);
    }
    ...
    /// <returns></returns>
    private static bool TileAction_ShowLivestockShop(
    ...
    )
    {
        ...
        if (
            AssetManager.BazaarData.TryGetValue(shopName, out BazaarData? shopData)
            && shopData.ShouldCheckShopOpen(who)
        )
        {
            // check opening and closing times
            if (
                (openTime >= 0 && Game1.timeOfDay < openTime)
                || (closeTime >= 0 && Game1.timeOfDay >= closeTime)
            )
            {
        ...
        // show shop
        return ShowShopDelegate(location, shopName, foundOwnerData);
    }
    ...
    )
    {
        try
        {
            ...
            ShowShopDelegate(__instance, AssetManager.MARNIE, null);
            return false;
        }
        catch (Exception err)
        {
            ModEntry.Log(
                $"Error in GameLocation_ShowAnimalShopMenu_Prefix:\n{err}",
                LogLevel.Error
            );
            return false;
        }
    ...
    /// <returns></returns>
    private static IEnumerable<CodeInstruction> PurchaseAnimalsMenu_ReturnToPreviousLocation_Transpiler(
    ...
    )
    {
            ...
            matcher
                ...
                .MatchStartForward(
                    [
                        ...
                        new(
                            OpCodes.Call,
                            AccessTools.Method(typeof(Game1), nameof(Game1.getLocationRequest))
                        ),
                    ]
                );
            matcher.Opcode = OpCodes.Call;
            matcher.Operand = AccessTools.PropertyGetter(
                typeof(OpenBazaar),
                nameof(ShopLocationName)
            );
        ...
        {
            ModEntry.Log(
                $"Error in PurchaseAnimalsMenu_ReturnToPreviousLocation_Transpiler:\n{err}",
                LogLevel.Error
            );
            return instructions;
        }
    ...
    private static IEnumerable<CodeInstruction> PurchaseAnimalsMenu_marnieAnimalPurchaseMessage_Transpiler(
        IEnumerable<CodeInstruction> instructions,
        ILGenerator generator
    )
    {
        ...
        {
            ModEntry.Log(
                $"Error in PurchaseAnimalsMenu_marnieAnimalPurchaseMessage_Transpiler:\n{err}",
                LogLevel.Error
            );
            return instructions;
        }
    ...
    private static NPC GetShopOwnerNPC(
        string _name,
    ...
    /// <returns></returns>
    public static bool ShowVanillaAnimalShop(
        GameLocation shopLocation,
        string shopName,
        ShopOwnerData? _owner
    )
    {
        ...
        {
            if (
                location.buildings.Any((Building p) => p.GetIndoors() is AnimalHouse)
        ...
        Game1.currentLocation.ShowPagedResponses(
            Game1.content.LoadString(
                "Strings\\StringsFromCSFiles:PurchaseAnimalsMenu.ChooseLocation"
            ),
            list,
                ...
                if (locationFromName != null)
                {
                    Game1.activeClickableMenu = new PurchaseAnimalsMenu(
                        AssetManager
                            ...
                            .Select(
                                (entry) =>
                                    new SObject("100", 1, isRecipe: false, entry.Data.PurchasePrice)
                                    {
                                        Name = entry.Key,
                                        Type = entry.AvailableForLocation(locationFromName)
                                            ? null
                                            : (
                                                (entry.Data.ShopMissingBuildingDescription == null)
                                                    ? ""
                                                    : TokenParser.ParseText(
                                                        entry.Data.ShopMissingBuildingDescription
                                                    )
                                            ),
